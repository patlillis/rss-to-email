...
async function checkRSSFeeds(env) {
  const parser = new Parser();
  const feedUrls = env.RSS_FEEDS.split(',');
  
  // Get the last check data from KV or create default
  let lastCheckData;
  try {
    const storedData = await env.BLOG_KV?.get(STORAGE_KEY, { type: 'json' });
    lastCheckData = storedData || { lastCheck: 0, seenEntries: {} };
  } catch (error) {
...
async function sendEmail(env, entries) {
